...
var projectionMeta = exports.projectionMeta = function projectionMeta(projection) {
  var paths = void 0,
      isExclusive = void 0,
      mixingTypes = void 0;
  ...
  if (type === 'string') {
    ...
    mixingTypes = paths.some(function (path) {
      return path.startsWith('-') === !isExclusive;
    });
    projection = paths.reduce(function (accumulator, path) {
      return _extends({}, accumulator, _defineProperty({}, isExclusive ? path.substring(1) : path, isExclusive ? 0 : 1));
    }, {});
  } else if (type === 'object') {
    paths = Object.keys(projection);
    isExclusive = projection[paths[0]] === 0;
    mixingTypes = paths.some(function (path) {
      return projection[path] === (isExclusive ? 1 : 0);
    });
  } else {
  ...
  return { isExclusive: isExclusive, projection: projection };
};
